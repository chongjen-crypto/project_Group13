<?php
/**
 * Notification bell + dropdown for staff/admin header.
 */
if (!isset($conn) || !isset($_SESSION['user_id']) || !in_array((string) ($_SESSION['role'] ?? ''), ['staff', 'admin'], true)) {
    return;
}
...
$staff_admin_notifications = notifications_fetch_for_user($conn, $notif_user_id, 25);
$staff_admin_notif_unread = notifications_unread_count($conn, $notif_user_id);
?>
<div class="notif-bell-wrap dropdown">
    ...
    >
        ...
        <?php if ($staff_admin_notif_unread > 0): ?>
            <span class="notif-badge" id="notifBadge"><?php echo $staff_admin_notif_unread > 99 ? '99+' : (int) $staff_admin_notif_unread; ?></span>
        <?php else: ?>
    ...
    <div class="dropdown-menu dropdown-menu-end notif-dropdown shadow-lg" aria-labelledby="btnNotifBell">
        ...
        <div class="notif-dropdown-body" id="notifDropdownBody">
            <?php if (empty($staff_admin_notifications)): ?>
                <div class="notif-empty text-muted">No notifications yet.</div>
            <?php else: ?>
                <?php foreach (array_slice($staff_admin_notifications, 0, 8) as $n): ?>
                    <div class="notif-item<?php echo (int) $n['is_read'] === 0 ? ' unread' : ''; ?>" data-id="<?php echo (int) $n['id']; ?>">
